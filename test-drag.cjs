...
async function test() {
  ...
  const addScreenBtn = await page.$('button');
  const buttons = await page.$$('button');
  let addBtn = null;
  for (const btn of buttons) {
    const text = await page.evaluate(el => el.textContent, btn);
    if (text && text.includes('Добавить экран')) {
      addBtn = btn;
      break;
    }
  }
  ...
  for (const btn of buttons2) {
    const text = await page.evaluate(el => el.textContent, btn);
    if (text && text.includes('Рисование зон')) {
      drawBtn = btn;
      break;
    }
  }
  ...
  const screens2 = await page.$$('[data-id]');
  const hotspots = await page.$$('[class*="hotspot"]');
  console.log('Found screens:', screens2.length, 'hotspots:', hotspots.length);

  if (hotspots.length > 0 && screens2.length >= 2) {
    const hotspot = hotspots[0];
    const targetScreen = screens2[1];
