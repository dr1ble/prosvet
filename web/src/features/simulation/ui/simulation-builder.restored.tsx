...
function parsePackageNameFromStoreUrl(value: string): string | null {
  ...
  if (host.includes("play.google.com")) {
    ...
    return id && isValidPackageName(id) ? id : null;
  }
  ...
  if (host.includes("rustore.ru")) {
    ...
    if (appIndex >= 0 && segments[appIndex + 1]) {
      const pkg = segments[appIndex + 1];
      return isValidPackageName(pkg) ? pkg : null;
    }
  }

  if (host.includes("apps.apple.com") || host.includes("itunes.apple.com")) {
    const pathMatch = parsedUrl.pathname.match(/\/id(\d+)/i);
    const rawId = pathMatch?.[1] ?? parsedUrl.searchParams.get("id");
    if (rawId?.trim()) {
      const appStorePackage = `appstore.id${rawId.trim()}`;
      return isValidPackageName(appStorePackage) ? appStorePackage : null;
    }
  }

  for (const key of ["id", "package", "packageName", "appId"]) {
    const candidate = parsedUrl.searchParams.get(key);
    if (candidate && isValidPackageName(candidate)) {
      return candidate;
    }
  }
...
export function SimulationBuilder({
    ...
    language === "ru"
      ? {
          ...
          targetAppExtractFromStoreSuccess:
            "Название, иконка и идентификатор приложения обновлены по странице магазина.",
      ...
      : {
          ...
          targetAppExtractFromStoreSuccess:
            "Application title, icon, and identifier were updated from the store page.",
          targetAppExtractFromStoreError:
  ...
  useEffect(() => {
    ...
    setDraft(localDraft);
    setSelectedScreenId(
      localDraft.startScreenId ?? localDraft.screens[0]?.id ?? null,
    );
    ...
    const hydrate = async () => {
      try {
        ...
        if (remoteDraft) {
          ...
          setSelectedScreenId(
            remoteDraft.startScreenId ?? remoteDraft.screens[0]?.id ?? null,
          );
          setSelectedHotspotId(null);
        }
      } catch {
  ...
  const handleExtractAppFromStoreUrl = async () => {
    ...
    try {
      ...
      const fallbackPackage = parsePackageNameFromStoreUrl(
        draft.targetApp.storeUrl,
      );
      if (fallbackPackage) {
        const detectedStoreType = detectStoreTypeFromUrl(
          draft.targetApp.storeUrl,
        );
        updateTargetApp({
  ...
  const isSimpleScreensStep = isSimpleMode && simpleStep === "screens";
                        ...
                        {mediaAssets.map((asset) => (
                          <article className={styles.mediaCard} key={asset.id}>
                            ...
                            <p className={styles.mediaMeta}>
                              {asset.contentType} ·{" "}
                              {formatSizeBytes(asset.sizeBytes)}
                            </p>
                            <ActionButton
